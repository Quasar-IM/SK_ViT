...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
    ...
    settings.checkpoint_project_path = 'train/{}'.format(config_name)
    if script_name_prv is not None and config_name_prv is not None:
        ...
        settings.checkpoint_project_path_prv = 'train/{}'.format(config_name_prv)
    ...
    if distill:
        ...
        if script_teacher is not None and config_teacher is not None:
            ...
            settings.checkpoint_project_path_teacher = 'train/{}'.format(config_teacher)
...
def main():
    ...
    if args.local_rank == -1 and env_local_rank is not None:
