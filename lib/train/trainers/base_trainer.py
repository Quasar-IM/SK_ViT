        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    # save every epoch from epoch 20 onward
                    save_every_epoch = getattr(self.settings, "save_every_epoch", False)
                    save_epochs = set(getattr(self.settings, "save_epochs", [10, 20, 30, 40, 50, 60, 80]))
                    save_epoch_range_start = int(getattr(self.settings, "save_epoch_range_start", 20))
                    in_save_range = (save_epoch_range_start > 0 and epoch >= save_epoch_range_start)
                    if in_save_range or save_every_epoch or epoch in save_epochs or epoch % 10 == 0:
        ...
        for key in fields:
            ...
            if key == 'net':
                try:
                    ...
                    print('Strict checkpoint load failed for net. Falling back to non-strict load.')
                    print(str(exc))
                    current_state = net.state_dict()
                    filtered_state = copy.deepcopy(checkpoint_dict[key])
                    removed_keys = []
                    if getattr(net, 'reinit_memory_neck_on_resume', False):
                        for state_key in list(filtered_state.keys()):
                            if state_key.startswith('memory_neck.'):
                                removed_keys.append((state_key, tuple(filtered_state[state_key].shape), 'reinit'))
                                del filtered_state[state_key]
                    for state_key, state_value in list(filtered_state.items()):
                        if state_key not in current_state:
                            continue
                        if current_state[state_key].shape != state_value.shape:
                            removed_keys.append(
                                (state_key, tuple(state_value.shape), tuple(current_state[state_key].shape))
                            )
                            del filtered_state[state_key]
                    if removed_keys:
                        print('Skipping checkpoint keys with shape mismatch:')
                        for state_key, ckpt_shape, model_shape in removed_keys:
                            if model_shape == 'reinit':
                                print(f'  {state_key}: checkpoint {ckpt_shape} skipped due to memory neck reinit')
                            else:
                                print(f'  {state_key}: checkpoint {ckpt_shape} vs model {model_shape}')
                    missing, unexpected = net.load_state_dict(filtered_state, strict=False)
                    if missing:
                        print(f'Missing keys during non-strict load: {missing}')
                    if unexpected:
                        print(f'Unexpected keys during non-strict load: {unexpected}')
            elif key == 'optimizer':
                try:
                    self.optimizer.load_state_dict(checkpoint_dict[key])
                except ValueError as exc:
                    print('Skipping optimizer state restore due to parameter-group mismatch.')
                    print(str(exc))
            else:
