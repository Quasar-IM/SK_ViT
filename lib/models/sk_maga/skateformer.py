...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, kernel_size=7, num_heads=32,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_hypergraph=False, hyperedges=None):
        ...
        if self.use_hypergraph:
            self.hgconv = HyperGraphBranch(
                channels=in_channels // (2 * 2),
    ...
    def forward(self, input):
        ...
        for i in range(self.gconv.shape[0]):
            ...
            y_gconv.append(z)
        y_graph = torch.cat(y_gconv, dim=1)
        if self.hgconv is not None:
            y_hgraph = self.hgconv(split_f_conv[0])
            y_graph = 0.3 * y_graph + 0.7 * y_hgraph
        y.append(y_graph)  # N C T V
...
class HyperGraphBranch(nn.Module):
    def __init__(self, channels, num_points, hyperedges):
        super().__init__()
        if num_points != 24:
            raise ValueError(f"HyperGraphBranch currently expects 24 points, got {num_points}")
        incidence = torch.zeros(num_points, len(hyperedges), dtype=torch.float32)
        for edge_idx, vertices in enumerate(hyperedges):
            for vertex in vertices:
                incidence[vertex, edge_idx] = 1.0
        edge_degree = incidence.sum(dim=0).clamp(min=1.0)
        vertex_degree = incidence.sum(dim=1).clamp(min=1.0)
        normalized_incidence = incidence / edge_degree.unsqueeze(0)
        hyper_adj = normalized_incidence @ incidence.t()
        hyper_adj = hyper_adj / vertex_degree.unsqueeze(1)
        self.register_buffer("hyper_adj", hyper_adj)
        self.proj = nn.Conv2d(channels, channels, kernel_size=1, bias=False)
        self.bn = nn.BatchNorm2d(channels)
        self.act = nn.GELU()

    def forward(self, x):
        agg = torch.einsum('vw,bctw->bctv', self.hyper_adj, x)
        return self.act(self.bn(self.proj(agg)))
...
class SkateFormerBlockDS(nn.Module):
    def __init__(
            ...
            use_hypergraph=False, hypergraph_scale=0.1, hyperedges=None):
...
class SkateFormerStage(nn.Module):
    def __init__(
            self, depth, in_channels, out_channels, first_depth=False, last_depth=False,
        ...
        for index in range(depth):
            blocks.append(
                SkateFormerBlockDS(
                    ...
                    num_points=num_points,
                    kernel_size=kernel_size,
                    downscale=((index == 0) and (not first_depth) and (not last_depth)),
                    num_heads=num_heads,
                    type_1_size=type_1_size,
                    type_2_size=type_2_size,
                    type_3_size=type_3_size,
                    ...
                    use_hypergraph=use_hypergraph,
                    hypergraph_scale=hypergraph_scale,
                    hyperedges=hyperedges))
...
class SkateFormer(nn.Module):
    def __init__(self, in_channels=3, depths=(2, 2, 2, 2), channels=(96, 192, 192, 192), num_classes=60,
                 ...
                 use_hypergraph=False, hypergraph_scale=0.1):
        ...
        hyperedges = [
            ...
            [18, 19, 6, 20],
            [21, 22, 12, 23],
            [6, 7, 8, 9, 10, 11, 20, 0, 1, 2, 3],
            [12, 13, 14, 15, 16, 17, 23, 0, 1, 2, 3],
            [6, 7, 8, 9, 10, 11, 20, 4],
            [12, 13, 14, 15, 16, 17, 23, 5],
            [20, 23, 0, 1],
        ]
        ...
        for index, (depth, channel) in enumerate(zip(depths, channels)):
            stages.append(
                SkateFormerStage(
                    ...
                    act_layer=act_layer,
                    norm_layer_transformer=norm_layer_transformer,
                    use_hypergraph=use_hypergraph and index == len(depths) - 1,
                    hypergraph_scale=hypergraph_scale,
                    hyperedges=hyperedges,
                )
            )
